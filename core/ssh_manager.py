...
class KaliSSH:
    ...
    def connect(self):
        if not self.is_connected or not self._is_channel_active():
            try:
                log.info(f"Estableciendo sesión con el entorno de explotación remoto ({self.ip})...")
                self._client.connect(self.ip, username=self.user, password=self.password, timeout=15)
                self.is_connected = True
                log.info("Sesión con el entorno de explotación establecida correctamente.")
            except Exception as e:
                log.error(
                    "No se obtuvo conectividad hacia el entorno de explotación remoto. "
                    "Verifique que el entorno esté activo y accesible en la red."
                )
                self.is_connected = False
                raise e
        return self
    ...
    def disconnect(self):
        if self.is_connected and self._client:
            try:
                self._client.close()
                log.info("Sesión con el entorno de explotación cerrada.")
            except Exception as e:
                log.error(f"Error al cerrar la sesión remota: {e}")
            finally:
                self.is_connected = False
                self._client = paramiko.SSHClient()
                self._client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ...
    def ejecutar(self, comando, timeout=None):
        self.connect()
        try:
            log.debug(f"Ejecutando instrucción en entorno remoto: {comando}")
            stdin, stdout, stderr = self._client.exec_command(comando, timeout=timeout)
            return stdin, stdout, stderr
        except Exception as e:
            log.error("La fase de explotación automatizada no pudo completarse por restricciones de conectividad.")
            self.is_connected = False
            raise e
    ...
    def upload_text(self, text, path_remoto):
        ...
        try:
            ...
            with sftp.file(path_remoto, "w") as f:
                f.write(text)
            sftp.close()
            log.debug(f"Recursos transferidos al entorno remoto: {path_remoto}")
        except Exception as e:
            log.error(f"No se pudieron transferir recursos al entorno remoto: {e}")
            raise e
